...
void rps_gui::on_push_button_start_clicked()
{
    ...
    ui->label_current_round->setStyleSheet("font-weight: bold; font-size: 16px");
    ui->label_current_round->setAlignment(Qt::AlignCenter);
...
void rps_gui::play_round(string human_choice) {
    ...
    ui->label_computer_prediction->setText("Computer predicted your move: " + QString::fromStdString(result[2]));
    ui->label_computer_choice->setText("Therefore, Computer chose: " + QString::fromStdString(result[1]));

    ui->label_winner->setText("WINNER: " + QString::fromStdString(result[0]));
    ui->label_winner->setStyleSheet("font-weight: bold; font-size: 16px");
    ui->label_winner->setAlignment(Qt::AlignCenter);
...
void rps_gui::update_stats() {
    ...
    ui->label_human_wins->setStyleSheet("font-weight: bold; font-size: 16px");

    ui->label_computer_wins->setText("Computer Wins: " + QString::number(stats[1]));
    ui->label_computer_wins->setStyleSheet("font-weight: bold; font-size: 16px");

    ui->label_ties->setText("Ties: " + QString::number(stats[2]));
    ui->label_ties->setStyleSheet("font-weight: bold; font-size: 16px");
}
